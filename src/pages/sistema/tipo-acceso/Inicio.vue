...
const {
    form, dato, tipoAccesos, errors, respuesta, tipoAcceso,
    listar, buscar, isActived, pagesNumber, cambiarPaginacion, cambiarPagina,
    limpiar, obtenerTipoAcceso, eliminarTipoAcceso, inhabilitarTipoAcceso,
    habilitarTipoAcceso
} = useTipoAcceso();
...
const nuevo = () => {
    ...
    (
        document.getElementById('modal-tipo-acceso-title')
    ).innerHTML ="Nuevo Tipo de Acceso";
...
const editar = (id) => {
    ...
    (
        document.getElementById('modal-tipo-acceso-title')
    ).innerHTML ="Editar Tipo de Acceso";

    $('#modal-tipo-acceso').modal('show')
}

const elimina = async (id) => {
    await eliminarTipoAcceso(id)
...
const eliminar = (id) => {
        ...
        title:'¿Está seguro de eliminar el Tipo Acceso?',
        icon:'question',
...
const inhabilita = async (id) => {
    await inhabilitarTipoAcceso(id)
    if(respuesta.value.ok==1) {
        errors.value=[]
        Toast.fire({
            icon: 'success',
...
const inhabilitar = (id) => {
        Swal.fire({
        //title: 'Grados Ministeriales',
        title:'¿Está seguro de inhabilitar el Tipo Acceso?',
        icon:'question',
        confirmButtonColor: "#28a745",
        confirmButtonText: "Si",
        showCancelButton: true,
...
const habilitar = (id) => {
        Swal.fire({
        //title: 'Grados Ministeriales',
        title:'¿Está seguro de habilitar el Tipo de Acceso?',
        icon:'question',
        confirmButtonColor: "#28a745",
        confirmButtonText: "Si",
        showCancelButton: true,
...
<template>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-sm">
                            ...
                            <tbody>
                                ...
                                <tr v-else v-for="(tipo,index) in tipoAccesos.data">
                                    <td v-text="index+tipoAccesos.from"></td>
    ...
    <TipoAccesoForm :form="form" @onListar="listar"></TipoAccesoForm>
</template>